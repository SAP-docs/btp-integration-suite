...
> 
> ```
> kubectl -n <istio gateway namespace> get service istio-ingressgateway
> 
> 
> ```

Which Istio gateway namespace should be used?

-   `istio-system`: used for the Standard option
-   `istio-gateways`: used for the Restricted Access option
...
1.  In your DNS service, create a new entry that associates the domain that you previously defined with the back-end address assigned to the ingress gateway.
    ...
    > ### Note:  
    > If you define an API Virtual Host \(optional\), create a second DNS entry for it.
    > 
    > Both DNS entries for the Default and the API Virtual Host must point to the same Ingress IP.
