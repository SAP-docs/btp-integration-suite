...
4.  Choose *Browse* and upload the XSD/WSDL file under the *XSD File*/*WSDL File* step.
    ...
    > 
    > Upload of Custom IDocs requires an XSD in the format as downloaded from Enterprise Service Repository \(ESR\) or from transaction *WE60* of your SAP S/4HANA backend system.
...
7.  For custom IDoc creation, the *Message Creation* step displays the following additional fields:
    ...
    > ### Note:  
    > With takt 2509, newly created custom IDoc message types \(or message type versions\) automatically include an additional hierarchy level called *IDOC* below the root node. You can set this node to repeating in your MIG and model bulk IDocs in Integration Advisor.
