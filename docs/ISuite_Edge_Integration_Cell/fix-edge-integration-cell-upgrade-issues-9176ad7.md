...
1.  To check the custom resource status, enter the command `kubectl get SolaceSoftwareBroker solace -n edge-icell-services` in the kubectl tool.
    ...
    Check which of the `ssb-solace-<n>` pods show status **Pending** or status **Running** with an incomplete **READY count**, for example 1/2 instead of 2/2. For these pods, enter the command `kubectl describe pod <name> -n edge-icell-services` in the kubectl tool.
    ...
    Confirm that `ssb-solace-<n>`pods show status **Running** and **READY count** 2/2.
