...
> 
> The API Virtual Host refers strictly to the custom domain name used to expose local API endpoints \(Public OData API\) of Edge Integration Cell. Do not use the host or configuration of your Kubernetes API server here.
> 
> Therefore, an additional Cloud Connector System Mapping is required for the API Virtual Host.
...
3.  Add a system mapping with the following values:
    ...
    -   **Internal Host:** `<API Virtual Host (Public OData API)>`
