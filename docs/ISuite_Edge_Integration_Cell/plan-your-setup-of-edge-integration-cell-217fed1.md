...
> ### Note:  
> We recommend using dynamic volume provisioning with the reclaim policy set to *Delete*. Specifically for the Message Service storage, you must use a clean, empty volume for the initial deployment. Reusing volumes that contain data from previous installations can cause initialization errors and deployment failures.
