...
-   **1:1**: Map one node from the source structure with one node of the target structure.

-   **N:1**: Map multiple nodes from the source structure with one node of the target structure. The values of all the mapped source nodes are merged into one date-time input and are fed into the target node.

The *Date Time Conversion* feature has the following feature variations based on the type of format:
...
-   If the format differs between the nodes, the format of the source node can be converted to the format of the target node during the mapping. To do so, you need to set the format of the nodes. Select the element and navigate to the *Functions* tab and choose *Switch to DateTime Mapping*.
    ...
    > 
    > If you want to implement your own function, you can do so using an XSLT code. To know more, see [Functions](functions-2ea22d0.md).
...
-   If source and target nodes are both Date, Time, or DateTime fields then the *DateTime Conversion* tab is displayed automatically. You see the default DateTime format of the source as well as of the target node which is set while defining the corresponding message implementation guidelines.
...
-   *Fixed Value*: This option allows you to provide a constant value for the missing parts of the target format.
    -   If you choose a **fixed value**, the field is always produced with this value, whether there's an input or not.
    -   If you choose a **fixed value for a UTC marker**, you can't define a *Timezone Offset*.
...
There could be scenarios where multiple source nodes provide the same input to the target node. In such cases, conflict of data could arise. Say for example, you have mapped 2 source nodes with one target node. The source nodes node1 and node2 both contain the format *MMYY* and the target date time format is also *MMYY*. This could result in a conflict. To resolve this, the application displays two separate fields below the *Target DateTime Format*:

-   *Month\(MM\)* 

-   *Year\(YY\)*

You can now choose which of the 2 source nodes will contribute to the MM part and which will contribute to the YY part using the drop-down list provided for the two fields mentioned before.

In other case, if node1 is of type *MM* and node2 is of type *DD* and the target node type is only *MM*, then only the data from node1 is picked and node2 is ignored.

*Adding Global Parameters for the Source Node*

You can also add global parameters for the source node. Suppose let us say you have created global parameters in the *Global Parameters* tab of your MAG that you wish to use in your datetime conversion. To do so , follow the following procedure:
