...
-   **Type-system based codelists** are provided by the type system itself. They can be used only in MIGs that are based on the same type system.

-   **Reusable codelists** are provided and maintained by organizations such as ISO or United Nations - Economic Commission for Europe \(UN/ECE\). They can be used in MIGs that are based on any type system. For example, the ISO-based codelists such as country code, language code, currency code, or the UN/ECE based codelist such as measure unit code. They're represented as separate type systems that only contain codelists.

-   **MIG codelists** are locally defined in a MIG by the users.
-   **Local codelists** are only defined in the context of a specific element and can't be used at any other place. They're predominantly used in the context of custom messages.
...
[Creating MIG Codelists](creating-mig-codelists-6318fe6.md "If the reusable or type-system based codelists don’t meet your requirements, you can create new message implementation guideline (MIG) codelists.")

[Working with Codelists](working-with-codelists-770f7be.md "Learn how to work with codelists in message implementation guidelines, edit code values, and assign codelists to leaf nodes.")
