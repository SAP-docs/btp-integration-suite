...
Using the SFTP sender adapter, you connect an SAP Cloud Integration tenant with an SFTP server so that the tenant can read data from the SFTP server \(in a process referred to as polling\).
