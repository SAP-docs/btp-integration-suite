...
5.  After the user gave their approval, the token service returns the authorization code of the user to the SAP Cloud Integration tenant \(Redirect URL\).
...
7.  The token service checks the request and sends a refresh token as a response. It also sends an access token that is ignored by the SAP Cloud Integration tenant \(Token Service URL with "authorization\_code" grant type is used\).
