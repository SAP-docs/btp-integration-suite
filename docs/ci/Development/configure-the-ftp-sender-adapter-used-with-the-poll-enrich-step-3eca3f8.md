...
The FTP sender adapter connects an SAP Cloud Integration tenant to a remote system using the TCP \(Transmission Control Protocol\) protocol to read files from the system. TCP File Transfer protocol is also referred to as File Transfer protocol \(or FTP\).
