...
2.  In the *Adapter Type* dialog, select the adapter that you want to assign.
    ...
    You see a list that includes both preshipped adapters and the ones from SAP Business Accelerator Hub. Though you haven't imported adapters from SAP Business Accelerator Hub, shows you all the adapters that are available from the Hub.

    After you select an adapter hosted on SAP Business Accelerator Hub, automatically imports the adapter and its package to your design workspace, and automatically deploys the adapter to the runtime profile configured for the integration flow.
