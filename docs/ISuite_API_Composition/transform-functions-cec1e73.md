...
The `rename` transform is the simplest and most common. As its name implies, the `rename` transform only provides an attribute renaming and a type conversion to supported attribute types. An optional type property overrides the default type conversion.

For more information, see [Attribute Types](attributes-3eb6109.md#loio38ca8c48464048efb22635f8b07fc78f).
...
The transform returns a localized string. A typical use is when the source API provides texts in all available languages, and the API Composition API only provides a single text in the `HTTP Content-Language` of the response. Implementation-wise, API Composition knows the preferred language of the client application user via the `HTTP Accept-Language` header in the query. This hides the complexity of language-specific string handling from the developer. When the `HTTP Accept-Language` header isn't in the request, `en` \(English\) is the assumed default.
