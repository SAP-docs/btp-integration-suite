...
This highlights the security aspects of using API Composition.

With the increasing use of distributed systems for managing business data, the demands on security are also on the rise. When using a new API, you need to be sure that it supports your business needs without allowing unauthorized access to critical information. User errors, negligence, or attempted manipulation of your system shouldn’t result in loss of information or processing time. These demands on security apply likewise to API Composition.

API Composition's security is ensured by the security model of SAP BTP. The following are governed by SAP BTP security principles and services:
...
The following table summarizes the main security features of API Composition:
...
Support the prevention of unidentified or anonymous access by client applications to data exposed via API Composition.
...
No access by unregistered applications to data exposed via API Composition.
...
Every service key of the API Composition instance has a different client secret.
...
API Composition is entirely stateless and doesn't store, cache, or otherwise keep copies of any business data.
...
Representation of a API Composition user by a business system technical user.
...
Business data access through the API Composition Navigator Application
...
The API Composition API and the business system endpoints are encrypted by applying `TLS.At-rest` encryption isn't required because API Composition doesn't persist or cache any sensitive business or personal data.
...
API Composition API endpoints are protected from \(D\)DoS \(L3/L4/L7\) attacks.

API Composition API endpoints are protected from overload \(Spike arrest\).
...
Software deployed to API Composition infrastructure is continuously scanned for known vulnerabilities. Only software whose identity is confirmed is deployed.
