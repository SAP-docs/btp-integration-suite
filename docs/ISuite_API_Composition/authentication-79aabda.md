...
To communicate with API Composition, client applications must present an access token, which they obtain from a security service during authentication: the process of verifying an identity \(such as a username\) of the user and the client by checking additional credentials \(such as a password\).

Client applications are not concerned with connections to the actual data sources – this is managed entirely by API Composition.

Client applications follow the OAuth 2.0 protocol to obtain an access token. API Composition supports two different scenarios, depending on the type of authentication flow required:
...
Either way, the retrieved access token must be sent to API Composition in every communication, by adding the following header to each `HTTP` call:
...
## Acquire API Composition Service Credentials

The authentication code of the client applications must use the following parameters to obtain an access token and to connect to API Composition:

-   The URL of the authentication service
-   Authentication credentials: `client id` and `client secret`
-   The API Composition API URL
