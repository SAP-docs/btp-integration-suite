...
-   Conceptual information you need to connect API Composition APIs to your applications.
...
-   Information about how to connect and use:
    ...
    -   [API Composition Navigator on Developer Hub](api-composition-navigator-on-developer-hub-8e75d31.md).
        ...
        This tool is available in API Composition on the SAP Integration Suite home page. You can inspect your configured business data graphs and try them out on your own system, including custom services, model extensions, and more.

    -   [API Composition Navigator](https://api.sap.com/api-composition) on SAP Business Accelerator Hub.

        A public listing of all supported entities from SAP business systems in a business data graph. You can try out the [API Composition APIs](https://api.sap.com/api/Graph_ConfigurationAPI/overview) using the provided sandbox system.

    -   [API Composition Syntax Diagrams](api-composition-syntax-diagrams-86101be.md) that give you a graphical representation of the APIs.
...
This is the problem that API Composition addresses.

API Composition is a  unified API for SAP. It addresses this run-away API complexity and integration challenge by providing developers a single connected and unified view of all their business data, consolidating the data models of data sources like SAP S/4HANA, SAP Sales Cloud, and SAP SuccessFactors into one, unified and connected data model of the data in a specific landscape. We call this the *Business Data Graph*.
...
![Business data graph showing relationships between SalesQuote, Customer, Product, and Division entities.](images/SalesQuote_Business_Data_Graph_ede046e.png)
...
With API Composition you navigate to and access the data you need, regardless of where this data resides. API Composition abstracts the physical landscape and the details of the different product stacks and offers you a simple view of the SAP-managed data, which you can access through a single API, spanning all key use cases. API Composition accesses the data in the customer-configured landscape on your users’ behalf, technically acting as middleware. API Composition itself doesn’t store or cache any data.

![Business Data Graph architecture diagram showing client apps connecting via GraphQL and OData v4 to data sources.](images/Graph_Landscape_62258b9.png)

The illustration shows your \(client\) app accessing quote, product, customer, and supplier data – but the app cannot tell if this data is accessed from an on-premise SAP S/4HANA system, SAP Sales Cloud, or another system. This is determined by the specific landscape configuration that API Composition accesses on behalf of the app. As a developer, you are exempted from knowing these system details, or how to access data in these heterogeneous systems. More importantly, your application is portable across multiple and diverse landscapes without changes to your code.
...
[API Composition Navigator](https://api.sap.com/api-composition) is the single source of information about the collection of over 2200 root entities that you can access via API Composition. It lists the unified entities, as well as the system-specific entities.
