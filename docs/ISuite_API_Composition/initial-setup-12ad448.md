...
As a Subaccount or Tenant Administrator, you need to add API Composition as a capability of API Management within SAP Integration Suite.
...
4.  You have subscribed to the SAP Integration Suite and assigned the *Integration\_Provisioner* role collection to yourself. You must assign this role to add capabilities, such as API Management and API Composition, on the SAP Integration Suite home page.
...
5.  You have activated API Composition on the SAP Integration Suite home page and assigned yourself to the following API Composition role collection:
...
## 1. Configure Entitlement for API Composition

Your global account administrator is responsible for configuring the API Composition entitlement as follows:

1.  Go to *Entitlements* in the SAP BTP cockpit.

2.  Choose *Edit* \> *Add Service Plans*.

3.  Search for and choose *API Composition* and choose the following:

    > ### Note:  
    > Some services are only available in the Cloud Foundry environment, so your subaccount must be Cloud Foundry-enabled.

    1.  *configuration* to configure business data graphs using the API Composition Configuration API.
...
## 2. Activate API Composition on SAP Integration Suite 

1.  Go to the *Integration Suite* home page, and under *Capabilities*, choose *Add Capabilities*.
2.  On the *Activate Capabilities* dialog, under *Select Capabilities*, choose *API Management* and choose *Next*.
3.  To activate API Composition, select the following, and choose *Next*:
    ...
    > ### Note:  
    > If you have already set up API Management, choose *Edit*, and select *API Composition*. You must select **Developer Hub** to activate *API Composition*.
...
## 3. Define Users for API Composition

API Composition doesn't manage user identities and it can't directly authenticate clients. Instead, it relies on the SAP Authorization and Trust Management service.
...
Once you enable API Composition, you see all of the API Composition role collections in your list.



### Enable API Composition Users

-   API Composition Key User
...
-   API Composition Navigator Viewer

    API Composition Navigator is a tool in Developer Hub that developers use to inspect business data graphs. For more information, see [API Composition Navigator on Developer Hub](api-composition-navigator-on-developer-hub-8e75d31.md).
...
-   API Composition Navigator Tryout

    To enable a user to explore business data graphs in the API Composition Navigator on the Developer Hub, assign the *GraphNavigator.Tryout* role as follows:
...
-   API Composition Guest

    There are cases where you need users to have read-only access to API Composition. Assign the *Graph\_Guest* authorization role to one or more users so that they have read-only access to API Composition and the API Composition Configuration API.
