...
Developers of client applications use API Composition to access data, without being aware of where it comes from, API Composition accesses the data on behalf of the developer.
...
In both situations, there might be more than one data source for a certain entity instance, creating a challenge for API Composition to solve. It doesn’t make assumptions about the enterprise landscape; instead, API Composition determines a specific data source for each search query or key-based reference, by following a business data graph locating algorithm. The algorithm is configured by the owner of the landscape, in the form of a set of locating rules and foreign keys that form a *locating policy*.
...
The most important aspect of a rule is the specification of a *leading system*: the data source that is considered the default source of truth for that entity type in the landscape. When the data changes, this is where it's changed. API Composition by default uses the leading system for search queries.
...
Another aspect of the locating policy is the specification of key mapping. Here, a key-based reference from system X refers to an entity in system Y, which is the leading system for that entity. The key identifying the instance from system X doesn't match the key of the entity in system Y. Therefore, API Composition needs to swap keys \(using key mapping\), to be able to access the entity in system Y. It's easy to think of such references as intersource foreign keys.
...
To implement the locality-of-reference principle, API Composition manages a key modifier, called a qualifier, which is a name for the data source. Qualified keys are used in unified entities and references. API Composition forms them by prefixing the qualifier to the system-specific local key, separated by a tilde character, for example, *s4EU~1356*. This is opaque to applications.

The locating algorithm consists of three steps where API Composition:
...
The result of this algorithm is always a single data source, and a translated key if a foreign key is found, or an error. API Composition doesn't support queries that would lead to searches over multiple data sources or that would require unions of data sets. Developers can implement such relatively infrequent and complex business logic in their own applications.

API Composition accesses the selected data source using its own native API. For this, it uses protocol conversions, advanced translations, and transform technology \(including type conversions and key mapping\) to access the data in the source system and present it to developers as a fully connected business.
