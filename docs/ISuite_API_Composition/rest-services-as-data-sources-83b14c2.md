...
API Composition allows you to compose business data graphs from multiple-source APIs \(called data sources\).

Next to OData, it supports REST-services as data sources. To the outside API Composition exposes an ODataV4 or GraphQL API for a business data graph, so that developers can consume all data via a single unified API with a single protocol. As generic REST-services can be designed and structured arbitrarily, API Composition transforms how resources from REST-services are exposed in a business data graph in order to ensure compliance with the external OData API.

> ### Note:  
> Customizing unbound actions and functions from REST data sources is not supported.
