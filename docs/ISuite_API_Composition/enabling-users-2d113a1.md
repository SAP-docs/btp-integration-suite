...
API Composition does not manage user identities and it cannot directly authenticate clients. Instead, it relies on the SAP Authorization and Trust Management service to do that. By setting a trust relationship to SAP Authorization and Trust Management, API Composition accepts the identities verified by SAP Authorization and Trust Management and passed to it in the form of bearer tokens. It supports token-based authentication.
...
After setting trust, SAP Authorization and Trust Management Service can authenticate users from the given IdP. Since API Composition has a trust relationship to SAP Authorization and Trust Management Service, it will be able to validate the tokens that it issued.
...
The creation and activation of business data graphs is managed by a user with a special role, called the API Composition key user.

The key user uses API Composition on the SAP Integration Suite home page to configure a business data graph. After activating API Composition as a capability of API Management within SAP Integration Suite, the role of `Graph_Key_User` must be assigned to a role collection in the SAP BTP cockpit.
...
If someone needs to review and understand how a business data graph is configured, but doesn't need permissions to make changes, then an administrator can assign them the role of `Graph_Guest`. A API Composition guest is given read-only access to API Composition and the API Composition Configuration API.

**Related Information**  


[Initial Setup](initial-setup-12ad448.md "As a Subaccount or Tenant Administrator, you need to add API Composition as a capability of API Management within SAP Integration Suite.")
