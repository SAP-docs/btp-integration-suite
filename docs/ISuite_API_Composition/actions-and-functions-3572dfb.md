...
Actions and functions provide a way to introduce server-side behaviors into the otherwise data-centric model. API Composition mirrors actions and functions as provided by connected business systems.
...
API Composition supports OData operations \(actions and functions\) and automatically discovers them in the metadata of connected business systems. Discovered actions and functions are mirrored into the resulting business data graph.
...
-   `bdg` = The business data graph identifier as specified by the API Composition configuration.
