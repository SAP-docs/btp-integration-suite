...
GraphQL is a query language for APIs that uses similar modeling principles to OData. It features a type system to describe the data model and gives clients the ability to specify the data they want to request. API Composition supports GraphQL as an alternative protocol to OData.
...
-   `bdg` is the business data graph identifier specified by the API Composition configuration.

Because the API Composition API is always protected by OAuth 2.0 authorization, the bearer token is sent with all of the requests in the `Authorization` header.
...
For the lastest list of limitations when using the GraphQL schema in API Composition, see [Limitations](https://github.com/cap-js/graphql?tab=readme-ov-file#limitations).
...
[The Graph OData V4 API](the-api-composition-odata-v4-api-79162b2.md "API Composition supports the OData v4 protocol to access business data graphs.")
