...
Connecting REST services as data sources to API Composition requires, unlike OData services, creating two destinations: a data destination and a metadata destination.

When integrating REST services into a business data graph, the resources of that REST service are transformed into unbound actions and functions to maintain OData compatibility. Read more about how resources from REST services are exposed in the [API Composition Configuration](https://api.sap.com/api/Graph_ConfigurationAPI/overview) OData API.
...
To integrate your REST services into API Composition, you'll need to create two destinations:

![](images/REST_Services_4f84cf4.png)
