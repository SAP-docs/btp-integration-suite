...
For Cross-Component Business Process Management \(ccBPM\) processes, Migration Assessment can identify the ccBPM name and the associated integration scenario. For Business Process Management \(BPM\) processes, Migration Assessment can only identify whether the integration scenario is using BPM, but it can’t identify the BPM name.

Additionally, Migration Assessment cannot read or analyze the internal BPM or ccBPM process logic. In this case, you may consider conducting a detailed analysis directly in the SAP Process Orchestration system, and have the migration effort estimated separately.
