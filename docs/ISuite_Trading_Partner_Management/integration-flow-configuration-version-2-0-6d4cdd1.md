...
The package **Cloud Integration – Trading Partner Management V2** contains the 2.0 version of the necessary artifacts that have to be configured. This integration flow uses the latest AS2 Sender Adapter 1.13 and AS2 MDN Sender Adapter 1.10 that provide you with more dynamic parameters.
...
To learn how to configure this profile mode, see [Creating a Trading Partner Profile](creating-a-trading-partner-profile-542fb11.md).

If you want to make your active agreement compatible with this version, follow the steps mentioned in [Migration Steps for 2.0 Compatibility](migration-steps-for-2-0-compatibility-8631960.md).
...
> 
> The default sender endpoint \(sender URL\) for integration flow 2.0 is the same as integration flow 1.x. If you want to deploy both 1.x and 2.0 version of the integration flow, you need to change the default sender endpoints to another value \(by configuring the externalized parameter\) in the 2.0 integration flow during deployment.
> 
> Consider the following points while using the ProcessDirect sender and receiver adapters:
> 
> -   The Groovy scripts provided in the integration package are reserved for *SAP* and it's not recommended to apply the Groovy code to custom integration flows.
> 
...
>     -   SAP\_S4HANA\_OnPremise\_SOA
>     -   GS1-XML
> 
> -   You can hand over all camel headers that are defined in your custom integration flows to Process Direct Step 1, and it's recommended not to use a camel header property name with the prefix *SAP\_* as the property value might get overwritten by the generic integration flow.
...
To copy the integration package to your *Design* space, perform the following steps.
...
6.  The following table lists the integration flows in the package and their purpose.
    ...
    > ### Note:  
    > To learn more about the integration flow step, see [Cloud Integration - Trading Partner Management V2 | SAP Business Accelerator Hub](https://api.sap.com/package/CloudIntegrationTradingPartnerManagementV2/documents).
    ...
    **Cloud Integration - Trading Partner Management V2**
    ...
    > ### Note:  
    > The following **sender adapters** are supported:
    > 
    > -   SOAP
    > 
    > -   AS2
    ...
    > -   ProcessDirect
    > 
    > The following **receiver adapters** are supported:
    > 
    > -   SOAP adapter
    > 
    > -   AS2 adapter
...
7.  To configure the integration flows, perform the following procedures:
