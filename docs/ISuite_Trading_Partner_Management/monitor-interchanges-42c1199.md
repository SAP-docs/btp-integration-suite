...
For details about roles and the tasks and permissions for the different personas, see [Identity and Access Management for Trading Partner Management](identity-and-access-management-for-trading-partner-management-5979108.md).
...
2.  By default, a table with the list of interchanges that were processed in the last 24 hours are displayed on screen. You can modify the list using the filter options provided above the table. There are multiple filter attributes that help you filter your required interchanges:
    ...
    **Processing Status**
    ...
    Filter interchanges according to their processing status.

    Interchanges can have one of following statuses:
    ...
    **Interchange Creation Time**
    ...
    Select from the following predefined time intervals during which the interchanges were created:
    ...
    **Relation Type**
    ...
    Filter interchanges by their relation type, which describes the source of the interchange, if any.

    Interchanges can be *Copied From* or *Split From* an original interchange, or be an *Original Interchange* themselves.
    ...
    **Original Interchange**
    ...
    Filter interchanges based on the document ID of their original interchange.

    This field only applies to interchanges that were copied or split from an original interchange, as designated by the column *Relation Type*.
...
6.  Choose :gear: above the table to view and add additional table column headers. In the *Columns* dialog, select/deselect the column headers and use <span class="SAP-icons-V5"></span> Up and <span class="SAP-icons-V5"></span> Down to reorder the columns in the table.

    You can also choose *Sort* to define custom sorting based on one or more columns that you select, which you can also move up and down by priority. Alternatively, directly choose a column header to sort the interchange list.

7.  To cancel interchange processing, select the checkboxes next to the interchanges that you want to cancel and choose *Cancel*. Interchanges can only be canceled if they're in one of the following statuses:
    ...
    > 
    >     > ### Note:  
    >     > Filtered results containing a large number of interchanges can prolong cancellation processing time.

    In the *Confirm Cancelation* dialog, provide a reason for canceling the interchange in the *Cancelation Reason* field and choose *Confirm Cancelation*.

8.  Choose <span class="SAP-icons-V5"></span> Sort to sort the table values in ascending/descending order based on a particular column header.
...
9.  To export the interchanges in Excel format, select the relevant entries and choose <span class="SAP-icons-V5"></span> Export to Spreadsheet.
    ...
    To customize the name of the exported spreadsheet, choose <span class="SAP-icons-V5"></span> Open Menu and *Export As...*. You can then edit the file name and decide whether to include the filter settings.

10. To download the payload of one or more interchanges, select one or more interchanges and choose <span class="SAP-icons-V5"></span> Download Payload \(Included Original Codepage\). You can also perform the same action over the context menu.

11. Select an interchange from the table to view it in detail. The interchange information is displayed to the right of the interchange table. The header area provides the following information about the interchange:
...
12. After the header section, the following sections are displayed:
    ...
    2.  *General*: Provides the general information such as the timestamp for process initiation and completion, interchange name and direction, agreement details, transaction type, and the following subsections:

        -   *Custom Attributes*: The custom attributes if defined in the agreement transaction are displayed here.
    ...
    3.  *Events*: The events section displays the *Monitoring Reference* link for the following messages:
        ...
        -   Technical Acknowledgment Sent \(for Received Sender Interchange\)
        ...
        -   Interchange is canceled \(applicable only for interchanges with *Canceled* and doesn't contain any reference link\)
    ...
    4.  *Interchange Payload*: View the payload information of *Sender Interchange Received* and *Receiver Interchange Assembled*. You can switch between these two payloads using the drop-down <span class="SAP-icons-V5"></span> provided for the *Version* field. Choose <span class="SAP-icons-V5"></span> Download Payload to download your payload information.

    5.  *Notes*: This section is only visible for interchanges that have been **Canceled**. The section mentions details of the cancellation, including:

        -   E-mail address of the user who initiated the cancellation

        -   The reason for cancellation \(as entered by the user\)
