...
2.  In the resulting screen, navigate to the tab *Partner Profiles*.
...
8.  In the *Create System* dialog, maintain the following fields:
    ...
    **Type**
    ...
    If your system type isn't available in the list, choose :heavy_plus_sign: and enter the details of the new system, then choose *Apply*. The newly created system is now selected in the *Type* field.
...
9.  Choose *Save*. The new system is created.
...
13. If you chose *Channel* as the configuration mode, you need to maintain the following tabs that appear below the fields:
    ...
    **Security Tab**
    ...
    Public Key Alias \(Only if you've selected *Trusted Certificate* or *Trusted Root Certificate*\)
...
14. If you chose *Profile* as the configuration mode, maintain the following tabs:
    ...
    > ### Note:  
    > The *AS2 Partner ID* tab is auto-filled based on the values that you provided in the previous fields.
...
17. If the newly created communication uses an **AS2 sender adapter**, you have to push its configuration to the partner directory before it can be used by an agreement. To push and activate your communication, change the switch button in the column *Activation Status* to *Active*.
...
<a name="loio49a6b02fa47247efa1707f4248fe22a5__result_cyk_x2y_1hc"/>

## Results

You've successfully created a communication partner.
