...
After creating your custom attribute, you need to assign these attributes to a transaction for it to reflect in the B2B monitor. To know more, see [Working with Trading Partner Agreements](working-with-trading-partner-agreements-9bd43c9.md).
...
Additionally, you can define the **behavior for duplicate messages**. This setting controls the idempotent processing behavior for message retries. To define the behavior, while in edit mode, select one of the following options for *Duplicate Messages*, then save your choice:

-   *Allowed* \(default\): Duplicate messages can be sent to the target system during automatic retries or manual restarts.
-   *Prevent*: The system uses the idempotent process capability in the generic integration flow to detect and skip duplicate interchanges, ensuring each message is processed only once.
...
13. Since your new custom rule is still in draft mode, you have to activate it to push its configurations to the Partner Directory.
    ...
    > ### Note:  
    > To activate custom rules, one of the following must apply:
    > 
    > -   You didn't edit the default keys and there's at least one custom key.
    > -   You edited at last one default key. In this case, you don't need a custom key.
...
> ### Note:  
> If a custom rule only changes the default keys and doesn't add a custom key, you don't have to assign this custom rule to an agreement. However, you still need to **activate** the custom rule.
