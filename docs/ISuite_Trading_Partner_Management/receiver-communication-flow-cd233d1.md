...
Perform the following procedure to configure the integration flow.

> ### Note:  
> If the receiver side integration flow calls a custom integration flow, only the computed Partner Directory ID header is handed over by the receiver integration flow to the custom integration flow.



## Procedure

1.  In the *Artifacts* tab, choose <span class="SAP-icons-V5"></span> Action for the integration flow *Step 3 - Receiver Communication Flow* and choose *Configure*.

2.  In the *Sender* tab, maintain the following parameter.
...
3.  Go to the *More* tab. There, set the value for the field *Maximum Retries Number*, which defines the maximum number of retries in case errors occur. By default, the value is `1`.

4.  Choose *Save*.

5.  Choose *Deploy*.

    The following table *Receiver Communication Flow* explains the integration flow process:
    ...
    **Receiver Communication Flow**
    ...
    This router checks the receiver adapter type. The message is sent depending on the adapter type to the receiver according to the properties and headers retrieved in PD Lookup in the interchange step. The following adapter types are supported:
    ...
    The following table `Retry Configuration` explains the retry configuration integration process:
