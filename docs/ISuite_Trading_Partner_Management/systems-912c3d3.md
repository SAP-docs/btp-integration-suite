...
*Systems* tab allows you to maintain the B2B standard details. You need this information to create agreements. Each agreement contains a specific B2B standard and conducts transactions based on the protocols provided by this B2B standard. To maintain the type systems in your agreement, you need to create and define them in the partner profile.
...
2.  Choose *Create* to add the following system details:
    ...
    **System**
    ...
    If your system type isn't available in the list, choose :heavy_plus_sign:, enter the details of the new system, and choose *Apply*. The newly created system is now selected in the *Type* field.
...
13. If you select AS2 as the *Sender* adapter, two new fields appear:
    ...
    -   *Security Configuration Mode*: Set your configuration mode to *Profile* or *Channel* from the drop-down list.
        ...
        > ### Note:  
        > It's highly recommended to use *Profile* mode as this mode supports reuse of configurations. *Channel* mode is only used for backward compatibility.
...
15. The following tabs appear if you choose **SFTP receiver adapter**. Maintain the following fields:
    ...
    ****
    ...
    If a file already exists in the target, the option allows to:
    ...
    If you select this option, the file exists check is performed on the SFTP server. If your server doesn't support this operation, switch back to client-side check.
...
16. Choose *Save*.
    ...
    > ### Note:  
    > Maintain both *Sender* and *Receiver* communication channel details to complete the system set-up. Once you've maintained the necessary details, the *Status* of the system is set to *Completed*.

17. If the newly created communication uses an **AS2 sender adapter** or an **SFTP sender adapter**, you have to push its configuration to the partner directory before it can be used by an agreement. To push and activate your communication, change the switch button in the column *Activation Status* to *Active*.
...
Next, you need to maintain the certificates. See [Certificates](certificates-5c4a01a.md).
