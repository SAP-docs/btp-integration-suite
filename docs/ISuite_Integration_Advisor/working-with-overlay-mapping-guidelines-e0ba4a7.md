...
# Working with Overlay Mapping Guidelines

Learn how to work with overlay mapping guidelines.
...
Overlay mapping guidelines automatically include all mapping elements from the underlying base mapping guideline.

> ### Note:  
> If you change a mapping element in the base mapping guidelines, this change is automatically considered in all overlay mapping guidelines in which this base mapping element is enabled.
...
4.  Overlay MAGs automatically inherit mappings from their base MAG, which are visible in the *Mapping* tab. You can manage the inherited base mappings by choosing to retain or delete them, and also add new mapping elements specific to this overlay MAG. Newly added mapping elements are displayed in green, while inherited elements from the base MAG appear in blue. The following options are applicable when mapping the source and target elements in an overlay MAG:
    ...
    See also [Matching Rules for Overlay Mapping Guidelines](matching-rules-for-overlay-mapping-guidelines-efb1ec1.md).
...
11. To delete overlay mapping elements and exclude base mapping elements in the overlay mapping guideline, select one or more mappings in the mapping list and choose *Delete/Exclude*.

12. You can add or manage additional information or functions about each element by selecting the mapping entity line.
...
13. You can combine an Overlay MAG with pretransformation \(PTS\). For this, you have the following options:
