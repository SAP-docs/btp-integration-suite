...
# Creating Overlay Graphical Mapping Guidelines
...
Overlay mapping guidelines are extensions of standard mapping guidelines that are used to fulfill specific business requirements.

To create an overlay MAG, perform the following steps.
...
1.  From the left pane of the application, choose *Design* \> *MAGs*.

2.  In the *Mapping Guidelines* page, choose *Create* \> *Graphical MAG: Overlay*.
...
4.  The *Source MIG* step displays the MIG that is used as the source in the selected base MAG. If you want to reuse the same source MIG in your overlay MAG, simply select the MIG.
    ...
    To use an alternative source MIG, refer to the *Compatible MIG* tab, which lists suitable replacement MIGs. Choose one from the list to proceed.
...
5.  Similarly, the *Target MIG* step displays the MIG that is used as the target in the selected base MAG. If you want to reuse the same target MIG in your overlay MAG, simply select the MIG.
    ...
    To use an alternative target MIG, refer to the *Use Compatible MIG* tab, which lists suitable replacement MIGs. Choose one from the list to proceed.

6.  The *MAG Creation* step displays the following information, most of which is automatically populated using the data from the base MAG:
...
7.  Review your choices, then choose *Create*.

    Once created, the newly created MAG has the label *Overlay* below its name to distinguish it from the standard MAGs.
...
To learn how to work with an overlay MAG, see [Working with Overlay Mapping Guidelines](working-with-overlay-mapping-guidelines-e0ba4a7.md).
