...
6.  In the *Code Values* tab of the codelist, you have the following options for adding code values:

    -   Choose *Add an Entry* to add an entry in the table with no values. You can then maintain the necessary details.
    -   Choose *Add an Entry* \> *Add from CSV file* to upload code values using a CSV file.

        In the resulting dialog, browse for and upload your CSV file by choosing *Select*.

        If you don't have a CSV file yet, see the section *Creating Codelists Using a CSV File*.

    -   Choose *Add from other codelists* to add code values from existing codelists in the system.

        In the resulting dialog, maintain the following:

        1.  *Type System*: Select a type system by choosing the <span class="SAP-icons-V5"></span> value help. In the resulting dialog, you can switch between *Standard* and *Custom* type systems, and select a type system from the list.

        2.  *Codelist*: Select a codelist from the list using the <span class="SAP-icons-V5"></span> value help.

        3.  *Version*: Select a codelist version from the list using the drop-down list.
    ...
    -   Choose *Add an entry* \> *Add empty code value* to add a code value with the value `(empty)`.

        You can't edit the value of this code value, but you can maintain a name and definition.


7.  Choose *Save* to save your changes.

8.  Choose *Activate* to activate your codelist. Once activated, the codelist becomes read only. To make new edits, create a new draft version of the codelist.

9.  To delete a codelist, see [Deleting a Custom Codelist](deleting-a-custom-codelist-01ad9ee.md).
...
2.  In the *Code Value Upload* dialog, choose *How to...* \> *Download the template\(\*.csv\)* option. This downloads the CSV file template locally with the name `CodeValueDownload.csv`.

3.  Edit the downloaded template file `CodeValueDownload.csv` to enter the required values and save.

    Ensure you don't change or remove the header of the template.
...
4.  Once done, you can choose *Select* to upload the updated file containing code values in the *Code Value Upload* dialog.
    ...
    > -   In the *Code Values* tab, choose *Download*. This downloads the standard codelist in `CSV` format.
