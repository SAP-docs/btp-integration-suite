...
Use the converter to convert XML message either to EDIFACT, ODETTE, TRADACOMS, VDA or ASC-X12 format.

XML to EDI Converter version 2.0 and above supports the TRADACOMS standard and version 2.9 and above supports the VDA standard \(*TRADACOMS* and *VDA* tab; available only for dedicated service plans, see [2903776](https://me.sap.com/notes/2903776)\).
...
2.  Define the parameters to convert the XML data format to EDI data format.
    ...
    **EDIFACT**
    ...
    > 
    > -   To retrieve the XSD file from Integration Advisor, see [Exporting XSD File from EDI Type System](https://help.sap.com/viewer/986600e940714dee8ed03f126ee7efca/CLOUD/en-US/5ab4cfe5ec724adda074c9773ea6b895.html "Export runtime artifacts for integration solutions.") :arrow_upper_right: or [Exporting Runtime Artifacts from MIG or MAG](https://help.sap.com/viewer/986600e940714dee8ed03f126ee7efca/CLOUD/en-US/5ab4cfe5ec724adda074c9773ea6b895.html "Export runtime artifacts for integration solutions.") :arrow_upper_right:.
    > 
    ...
    **VDA**
    ...
    > ### Note:  
    > -   The converter supports flexible cross-structure conversions across different XML structures and XSD schema types. When both PI/PO and IA XSDs are uploaded as input, the converter automatically prioritizes IA XSDs for processing.
    > -   The XML structures for IA and PI/PO XSDs differ significantly. If both XSD types are configured and the incoming XML payload conforms to the PI/PO structure, the conversion fails due to an XSD mismatch. Ensure the incoming XML payload conforms to the structure of the selected XSD.
    > -   To process PI/PO XML payloads, configure only the PI/PO XSD. If both XSD types are configured, the incoming XML payload must conform to the IA XML structure.
    ...
    *Source Encoding* 
    ...
    Use the appropriate encoding format of the inbound VDA interchange.
    ...
    -   Dynamic
        ...
        If selected, the encoding format is set dynamically at runtime using the header `SAP_XMLTOEDI_VDA_SOURCE_ENCODING` \(possible values: `UTF-8`, `ISO-8859-1`\).
    ...
    *Target Encoding* 
    ...
    Select encoding format for the outgoing payload. If you select *Dynamic*, you must define the value `UTF-8` or `ISO-8859-1 to ISO-8859-9` in `SAP_XMLTOEDI_VDA_TARGET_ENCODING` exchange header. You can also select encoding format from the incoming payload.
    ...
    *Conversion Preference* 
    ...
    Specify the conversion preference for the VDA message.
    ...
    -   Dynamic
        ...
        If selected, this parameter is set dynamically at runtime using the header `SAP_XMLTOEDI_VDA_CONVERSION_PREFERENCE` \(possible values: `No`, `Yes`\).
    ...
    -   Dynamic
        ...
        If selected, this parameter is set dynamically at runtime based on the value of the header `SAP_XMLTOEDI_VDA_SCHEMA_SOURCE` \(possible values: `Header`, `IntegrationProject`\).
    ...
    > ### Note:  
    > The XSD file must come from either SAP Process Integration/SAP Process Orchestration or Integration Advisor. Apply the following naming convention for the file name:
    > 
    > -   SAP Process Integration/SAP Process Orchestration XSDs: Example: `VDA490500.xsd`
    > 
    > 
    > -   Integration Advisor XSDs: Example: `VDA: VDA_VDA4905.xsd`
    ...
    *Segment Terminator*
    ...
    \(Available from version 2.9 and above\)
    ...
    -   *Dynamic*: If you select this option, you must define the value in `SAP_XMLTOEDI_VDA_SEGMENT_TERMINATOR` exchange header.

    -   *Carriage Return*: This option inserts a carriage return character at the end of each segment.

    -   *Line Feed*: This option inserts a line feed character at the end of each segment.

    -   *None*: No explicit segment terminator character is added.
