...
-   To set up NFS for Windows Server, see [Deploy NFS](https://learn.microsoft.com/en-us/windows-server/storage/nfs/deploy-nfs).
    ...
    > ### Note:  
    > Create or use \(if already present\) dedicated UIDs/GIDs for valid users. These UIDs/GIDs should have the appropriate access to your NFS server. For more information, see [NFS Authentication](https://learn.microsoft.com/en-us/windows-server/storage/nfs/deploy-nfs?tabs=gui#nfs-authentication).
