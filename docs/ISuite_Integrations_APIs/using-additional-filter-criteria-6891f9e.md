...
Use the *Filters* option to control which message processing logs are displayed .

The dialog that opens provides two views: *List* and *Group*. The *List* view displays all active filters individually; the *Group* view organizes them by category. The *Edit* button allows you to modify filter configurations, and the *Sort* button changes the display order of the filters.

You can use the *Search for Filters* field to locate a specific filter by name and use the *Add Filter* to add new filters to the list.

![](images/MPL_Filter_172ecb4.gif)
