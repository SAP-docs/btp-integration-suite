...
If you are using Edge Integration Cell or Integration Cell as your runtime profile, you can add an additional request reply policy to an API artifact to call an external HTTP\(S\) endpoint.
...
By default, only one request reply policy is created when you create an API artifact. This initial request reply policy cannot be deleted. However, you can add more request reply policies to the request/response flow, enabling multiple service callouts. See, [Supported Adapters for External Callout in API Artifacts](supported-adapters-for-external-callout-in-api-artifacts-641600e.md).
