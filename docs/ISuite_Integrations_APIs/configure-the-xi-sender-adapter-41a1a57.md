...
-   *User Role*: Sender authorization is checked based on roles defined on the tenant for the user associated with the inbound request. Choose *Select* to get a list of all available roles.
    ...
    > ### Note:  
    > These examples are not the only supported options. For a list of supported authentication methods for HTTP-based inbound communication, refer to [Authentication Options \(Inbound\)](40-RemoteSystems/authentication-options-inbound-5495ee0.md).
