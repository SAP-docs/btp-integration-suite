...
## Timeout Behavior for HTTP communication
...
-   When considering inbound communication through HTTP-based sender adapters, note that in particular the load balancer has its own timeout setting that influences the overall timeout. For inbound scenarios, SAP recommends that you design them so no communication is expected to wait longer than 10 minutes. In general, it is recommended to keep waiting times as short as possible to avoid unnecessary load on the network components.

    In inbound scenarios, Cloud Integration uses a Java web server to receive and process inbound HTTP requests. This web server has an important timeout parameter that controls how long it is allowed to wait while reading the request body. Cloud Integration sets this parameter to 20 seconds \(to mitigate the risk of DoS attacks with fragmented HTTP requests\). If the delay between the URI and the HTTP request body, the delay between body chunks \(in case of chunked transfer encoding\), or the delay between individual network packets delivering the HTTP request body exceeds this timeout, the request will be cancelled and an error message returned to the client.
