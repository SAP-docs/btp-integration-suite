<!-- loio13a0318d95c042ffae649fd0aaa9f35f -->

# Importing Message Type from the Enterprise Service Repository

Understand how to import a message type from ESR Repository.



<a name="loio13a0318d95c042ffae649fd0aaa9f35f__prereq_tcl_g2x_3xc"/>
...
> ### Note:  
> For visual instructions on how to create message types, refer the tutorial [Creating data types and message types.](https://developers.sap.com/tutorials/btp-integration-suite-creatingdatatype-messagetype.html)



## Procedure

1.  Navigate to the *Artifacts* tab and choose *Add* \> *Message Type* to add a message type artifact.
...
4.  Choose *Connect*. A list of available message types in ES repository appears.

5.  Select the message type that you wish to import.

6.  Select the *Include Dependent Message Types* checkbox if you wish to include the dependent message type objects while importing the selected message type.
...
7.  Choose *View Summary* to view a summary of all the artifacts to be imported. From the list of objects, you can view the following object status in the *Information* column:
    ...
    -   *Error; ID already exists:* Means that an artifact with the same ID as the selected primary message type already exists in an integration package. Choose *Resolve*, to assign another ID for this artifact. Then, choose *View Summary*.

8.  Choose *Add*. Import results with the details of created, skipped, failed artifacts appear.

    The artifact is added to the *Artifacts* tab. To further edit the message type, open the artifact in its editor and see [Configuring Data Types](configuring-data-types-97ad101.md).
