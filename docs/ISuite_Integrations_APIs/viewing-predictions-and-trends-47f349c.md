...
> ### Note:  
> Predictions is only available for API artifact types. MCP artifact types are not currently supported. When you select *MCP Server* as the artifact type, the **Predictions** page will display a message indicating that this feature is not available for the selected artifact type.
