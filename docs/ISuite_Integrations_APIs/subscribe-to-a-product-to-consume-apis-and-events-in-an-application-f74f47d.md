...
# Subscribe to a Product to Consume APIs and Events in an Application

As an application developer, you need to subscribe to a product to consume the APIs and Events within it. As part of the subscription process, you can either create a new application and add the product to it, or add the product to an existing application.
...
3.  Choose *Subscribe*.
    ...
    -   Create a new subscription for application from the Developer Hub catalog:
        ...
        1.  In the *Create New Subscription for Application* dialog box, enter a *Title*, *Short Text*, a *Description* \(optional\), and a *Callback URL* \(optional\) for the application.
            ...
            For event products, select a *Rate Plan*.

            > ### Note:  
            > Selecting a rate plan is mandatory when creating a subscription for an event product.
    ...
    2.  Select API or event as the *Product Type*, find and select the product that you want to subscribe to, and, for event products, select a *Rate Plan*. Then, choose *Next*.
