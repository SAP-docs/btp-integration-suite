...
-   Adapter Specific Message Attributes \(ASMAs\): If the imported mapping uses ASMA, you must manually create equivalent headers or properties in a Content Modifier or a Groovy script. These values should then be stored in the message headers/properties and used later in the message mapping or Groovy logic as required. See [Import a Message Mapping from Enterprise Services Repository \(ESR\)](https://help.sap.com/docs/integration-suite/integrations-and-apis-9519789d5664487f8b9cd89eba514477/creating-message-mapping-as-artifact?state=CLOUD#import-a-message-mapping-from-enterprise-services-repository-(esr))
