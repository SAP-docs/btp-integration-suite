...
# Govern and Manage an MCP Server Created Using an HTTP Endpoint
...
-   The **Source** section displays the connection details used to connect to the external MCP server.
-   Depending on how the MCP server was created, the source is either an **HTTP endpoint URL** or an **SAP BTP destination**.
-   You can edit the endpoint URL or select a different destination to update the connection.
