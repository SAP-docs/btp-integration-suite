...
# Modify an OpenAPI Specification Using the Code Editor
...
## Mandatory Fields for OpenAPI Spec

The following fields are required when defining an OpenAPI specification for a provider-based or URL-based API artifact.

1.  **OpenAPI / Swagger Version**

    The spec must declare a supported version:

    -   **Swagger 2.x:** swagger: "2.0"
    -   **OpenAPI 3.x:** openapi: 3.x.y \(e.g., openapi: 3.1.0\)

2.  **info Object**

    The info object must include the following valid fields:

    -   **Common Fields \(Required for All Artifacts\)**
    ...
    -   **For Provider-Based API Artifacts**
        ...
        **Requirement**
        ...
        Mandatory; must be a valid destination name
...
3.  **Host / Server Configuration**
    ...
    -   OpenAPI 3.x
        ...
        The servers object must include a`url` field that represents a valid virtual host URL appended with the basePath.

        servers: - `url: https://<virtual-host>/<basePath>`

    -   Swagger 2.x

        Both host and basePath fields must be defined and represent a valid virtual host and base path, respectively.
...
4.  **paths Object**
    -   The paths object is mandatory and must be defined—even if empty.
    -   A spec can be saved without the paths field, but the editor will flag it as an error.
