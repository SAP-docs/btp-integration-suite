...
# Add Policies to Artifacts

To enforce security or control traffic, you can set rules on the artifacts \(APIs and MCP servers\) by adding policies and integration steps to it.
...
-   You have a thorough understanding on policies and the various flows it can be attached to. For more information, see [Working with Policies and Mediation Steps](working-with-policies-and-mediation-steps-c744df5.md).
...
Add a policy to an artifact you've created.

You can also add integration flow steps to the artifacts. To know more about configuring the flow steps, see [Configure Integration Flow Components](configure-integration-flow-components-3171795.md).
...
3.  Select the *<integration package\>* where the artifact is created and choose *Edit*.

4.  Select the artifact within the package, then go to the *Policies* tab and choose *Edit*.

    Use *Policy Model* to add a policy to the artifact.

    You can add both traffic management and security policies to the API artifact/MCP server.
